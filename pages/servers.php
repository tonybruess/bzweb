...
if ($_POST['createserver'] == "1" && $_POST['newserver'] != ""){ // form was posted and isnt blank
	...
	$e_ar = mysql_fetch_array($e);
		...
		if($e_ar[0] == "$newserver"){
			...
			mysql_query("INSERT INTO servers (`master`, `name`, `owner`, `enabled`) VALUES ('$g_ar[0]', '$newserver_clean', '$g_ar[2]', '1')");
			}
...
if ($_POST['creategroup'] == "1" && $_POST['newgroup'] != ""){ // form was posted and isnt blank
	...
	if($g_ar[0]){
			...
			if(!$_POST['underuser']){
				mysql_query("INSERT INTO groups (`name`, `owner`, `enabled`) VALUES ('$newgroup_clean', '$name', 1)");
				echo "Group Created Successfully!";
			}
			if($_POST['underuser']&&$_SESSION['perm'][8]){
				$underuserc = sanitize($_POST['underuser']);
				mysql_query("INSERT INTO groups (`name`, `owner`, `status`, `enabled`) VALUES ('$newgroup_clean', '$underuserc', 0, 1)");
